...
int pos(_csp *csp,int partida,int destino){
    int i;
    for(i=0;i<(int)csp->graph[partida].size();i++){
        if(csp->graph[partida][i].dest==destino){
            return i;
        }
    }

    assert(0 && "Failed to find the index!\n");
...
void chvatal_heuristic(_csp *csp, std::vector<_journey> &journeys){
    ...
    while(csp->N != n_tarefas_ja_cobertas){
        ...
        if(verificador){
            verificador = false;
            for(i=0; i<(int)csp->start_nodes.size(); i++){
                std::vector<int> tarefas_percorridas;
                if(!pertence(tarefas_ja_cobertas,csp->start_nodes[i])){
                    verificador = true;
                    pega_todas_as_jornadas(csp, &melhor_jornada,csp->start_nodes[i],tarefas_percorridas,tarefas_ja_cobertas, &menor_custo_relativo);
                }
            }
        }else{
            for(i=0; i<(int)csp->graph.size(); i++){
                std::vector<int> tarefas_percorridas;
        ...
        for(i=1; i<(int)melhor_jornada.covered.size(); i++){
...
void pega_todas_as_jornadas(_csp *csp, _journey *jornada_atual, int no_atual, std::vector<int> tarefas_percorridas, std::vector<int> tarefas_ja_cobertas, double *menor_custo_relativo){
    ...
    for(i=1;i<(int)tarefas_percorridas.size();i++){
    ...
    for (i=0; i<(int)csp->graph[no_atual].size(); i++){
